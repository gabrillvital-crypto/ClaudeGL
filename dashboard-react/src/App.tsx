...
export function App() {
  ...
  const kpis = useMemo(() => {
    ...
    return {
      ...
      docs_aprovados:          sit.filter(r => r.Status === 'Aprovado').length          + forn.filter(r => r.Status === 'Aprovado').length,
      docs_reprovados:         sit.filter(r => r.Status === 'Reprovado').length         + forn.filter(r => r.Status === 'Reprovado').length,
      docs_nao_enviados:       sit.filter(r => r.Status === 'Não anexado').length       + forn.filter(r => r.Status === 'Não Anexado').length,
      docs_aguard_sub:         sit.filter(r => r.Status === 'Aguardando Submissão').length,
      docs_em_analise:         sit.filter(r => r.Status === 'Em Análise').length        + forn.filter(r => r.Status === 'Em análise').length,
  ...
  const r4Kpis = useMemo(() => {
    ...
    const nao_conf = reprovado + nao_anex + em_anal + vencido
    return {
      r4_total:        total,
