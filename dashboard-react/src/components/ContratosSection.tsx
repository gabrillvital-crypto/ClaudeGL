...
function Level3({ contrato, onBack }: { contrato: ContratoItem; onBack: () => void }) {
  return (
    <div>
      <button onClick={onBack}
        className="bg-[#e8f7fa] text-[#0E8FA3] cursor-pointer font-bold text-sm px-2.5 py-1 rounded hover:bg-[#d0eff5] mb-4 border-0">
        ← {contrato.codigo}
      </button>
      <div className="text-[12px] text-[#888] mb-3">
        ...
        {contrato.aeroporto && <span className="ml-2 bg-[#e8f7fa] text-[#0E8FA3] px-2 py-0.5 rounded font-semibold">{contrato.aeroporto}</span>}
      </div>
          ...
          <tbody>
            {contrato.terceiros.map((t, i) => (
              <tr key={i} className={`border-b border-[#e5eef1] hover:bg-[#f0f8fa] ${i % 2 === 1 ? 'bg-[#f8fafb]' : ''}`}>
                ...
                <td className="px-3 py-2 font-mono text-[12px] text-[#555]">{fmtCPF(t.cpf) || '—'}</td>
    ...
    return (
      <div>
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <button onClick={onBack}
            className="bg-[#e8f7fa] text-[#0E8FA3] cursor-pointer font-bold text-sm px-2.5 py-1 rounded hover:bg-[#d0eff5] border-0">
            ← Fornecedores
          </button>
        ...
        <Level3 contrato={selContrato} onBack={() => setSelContrato(null)} />
      </div>
    )
  }
      ...
      <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))' }}>
        {data.map(f => (
          <div key={f.cnpj} onClick={() => setSelForn(f)}
            ...
            <div className="text-[11px] text-[#888] mt-1">{f.totalTerceiros} terceiro{f.totalTerceiros !== 1 ? 's' : ''} vinculado{f.totalTerceiros !== 1 ? 's' : ''}</div>
            <div className="mt-2.5 flex gap-1 flex-wrap">
              {f.contratos.slice(0, 3).map(c => (
                <span key={c.codigo} className="text-[10px] bg-[#e8f7fa] text-[#0E8FA3] px-1.5 py-0.5 rounded font-medium truncate max-w-[120px]" title={c.codigo}>
                  {c.codigo}
                </span>
              ))}
