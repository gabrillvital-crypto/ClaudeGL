...
import { fetchTasksToday, fetchTasksOverdue, setTaskStatus } from '../lib/firebase'
...
export default function TabHoje({ onOpenTask, refresh }) {
  ...
  if (loading) return <p className="text-center text-gray-400 mt-8 text-sm">Carregandoâ€¦</p>
  ...
  return (
    <div className="px-4 pb-4 pt-3 overflow-y-auto h-full">
      {empty && (
        <p className="text-center text-gray-400 mt-12 text-sm">Nenhuma tarefa para hoje ðŸŽ‰</p>
      )}

      {overdue.length > 0 && (
        <>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs font-bold text-red-500 uppercase tracking-wide">âš  Atrasadas</span>
      ...
      {today.length > 0 && (
        <>
          <div className={`flex items-center gap-2 mb-2 ${overdue.length ? 'mt-4' : ''}`}>
            <span className="text-xs font-bold text-orange-500 uppercase tracking-wide">ðŸ“… Vencem hoje</span>
